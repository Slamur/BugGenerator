...
public class BugAcmp implements Runnable{
...
//    private final boolean ONLINE_JUDGE = System.getProperty("ONLINE_JUDGE") != null;
    ...
    private void solve() {
        ...
        long result = getResult(n, m, field);
        out.println(result);
    }

    public static final char WALL = '@', EMPTY = ' ';

    public static long getResult(int n, int m, char[][] field) {
        boolean[][] wall = new boolean[n][m];
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                wall[i][j] = field[i][j] == WALL;
            }
        }

        BugStepsCalculator calculator = new BugStepsCalculator(n, m);

        return calculator.getResult(wall);
    }

    static class BugStepsCalculator {
//        public static boolean checkPath(boolean[][] wall, int n, int m, int startX, int startY, int endX, int endY) {
//            DSU dsu = DSU.createInstance(n * m);
//
//            for (int i = 0; i < n; ++i) {
//                for (int j = 0; j < m; ++j) {
...
//            }
//
//            return dsu.connected(startX * m + startY, endX * m + endY);
//        }
        ...
        BugStepsCalculator(int n, int m) {
            this.n = n;
            this.m = m;
            ...
            this.queueX = new int[n * m + 10];
            this.queueY = new int[n * m + 10];

            this.colors = new long[n][m];
            this.color = 0;

            this.counts = new int[n][m];
            Arrays.fill(counts[0], INF);
            Arrays.fill(counts[n - 1], INF);
        ...
        public boolean checkPath() {
            ...
            colors[startX][startY] = color;
            queueX[tail] = startX;
            queueY[tail] = startY;
            ++tail;

            while (head < tail){
                int fromX = queueX[head], fromY = queueY[head];
                ++head;

                for (int[] step : steps) {
                    int toX = fromX + step[0], toY = fromY + step[1];

                    if (wall[toX][toY]) continue;
                    if (colors[toX][toY] == color) continue;
            ...
            return colors[endX][endY] == color;
        }

        public int getResult(boolean[][] wall) {
            this.wall = wall;
        ...
        public int getSteps() {
            ...
            for (int i = 1; i < nInner; ++i) {
                for (int j = 1; j < mInner; ++j) {
                    counts[i][j] = (wall[i][j] ? INF : 0);
                }
            }

            for (int x = startX, y = startY, direction = -1; x != endX || y != endY; ++result) {
                counts[x][y]++;

                int smallestDir = -1;
                int smallestDirCount = INF;
                ...
                for (int dir = 0; dir < steps.length; ++dir) {
                    ...
                    int xx = x + step[0], yy = y + step[1];

                    int dirCount = counts[xx][yy];
                    if (dir == direction) curDirCount = dirCount;

                    if (smallestDirCount > dirCount) {
                        smallestDir = dir;
                        smallestDirCount = dirCount;
                    }
                }

                if (curDirCount == smallestDirCount) {
                    smallestDir = direction;
                }

                x += steps[smallestDir][0];
                y += steps[smallestDir][1];

                direction = smallestDir;
            }
    ...
    public static void main(String[] args){
        new Thread(null, new BugAcmp(), "", MAX_STACK_SIZE * (1L << 20)).start();
    }
